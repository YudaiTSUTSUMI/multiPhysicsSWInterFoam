...
#include "modifiedInverseDistanceCellCellStencil.H"
...
{
    defineTypeNameAndDebug(modifiedInverseDistance, 0);
    addToRunTimeSelectionTable(cellCellStencil, modifiedInverseDistance, mesh);
}
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

Foam::label Foam::cellCellStencils::modifiedInverseDistance::index
(
...
Foam::labelVector Foam::cellCellStencils::modifiedInverseDistance::index3
(
    const labelVector& nDivs,
    const label boxI
)
...
Foam::point Foam::cellCellStencils::modifiedInverseDistance::position
(
    const boundBox& bb,
    const labelVector& nDivs,
    const label boxI
...
void Foam::cellCellStencils::modifiedInverseDistance::fill
(
    PackedList<2>& elems,
    const boundBox& bb,
    const labelVector& nDivs,
...
void Foam::cellCellStencils::modifiedInverseDistance::markBoundaries
(
    const fvMesh& mesh,
...
bool Foam::cellCellStencils::modifiedInverseDistance::overlaps
(
    const boundBox& bb,
    const labelVector& nDivs,
...
void Foam::cellCellStencils::modifiedInverseDistance::markPatchesAsHoles
(
...
bool Foam::cellCellStencils::modifiedInverseDistance::betterDonor
(
...
void Foam::cellCellStencils::modifiedInverseDistance::markDonors
(
...
//void Foam::cellCellStencils::modifiedInverseDistance::uncompactedRegionSplit
//(
//    const fvMesh& mesh,
...
//Foam::cellCellStencils::modifiedInverseDistance::compactedRegionSplit
//(
//    const fvMesh& mesh,
...
void Foam::cellCellStencils::modifiedInverseDistance::findHoles
(
    const globalIndex& globalCells,
    const fvMesh& mesh,
    const labelList& zoneID,
...
void Foam::cellCellStencils::modifiedInverseDistance::stencilWeights
(
    const point& sample,
...
void Foam::cellCellStencils::modifiedInverseDistance::holeExtrapolationStencil
(
    const globalIndex& globalCells
)
{
...
void Foam::cellCellStencils::modifiedInverseDistance::createStencil
(
    const globalIndex& globalCells,
...
Foam::cellCellStencils::modifiedInverseDistance::modifiedInverseDistance
(
    const fvMesh& mesh,
...
bool Foam::cellCellStencils::modifiedInverseDistance::update()
{
    ...
    // allCellTypes in overset boundary cell should be hole?
    labelList myAllCellTypes = allCellTypes;
    {
        const fvBoundaryMesh& pbm = mesh_.boundary();

        forAll(pbm, patchI)
        {
            const fvPatch& fvp = pbm[patchI];

            if (fvp.type() == "overset") // "overset" is constraintType
            {
                const labelList& fc = fvp.faceCells();
