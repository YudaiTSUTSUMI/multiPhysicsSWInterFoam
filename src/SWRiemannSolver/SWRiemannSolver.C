...
)
{
    ...
    scalar wetdryLeftCheck = wetdryLeft;
    scalar wetdryRightCheck = wetdryRight;
    ...
    if((wetdryLeftCheck == 0.0 && wetdryRightCheck == 1.0 && h0Left > hTotalRight) || (wetdryLeftCheck == 1.0 && wetdryRightCheck == 0.0 && h0Right > hTotalLeft))    
    {
        qLeft = 0.0; 
        qRight = 0.0;
    }
    ...
    //calculate friction source term
		...
		scalar SFriction = -dX*Cf*qroe*mag(qroe);
    ...
    if(mag(denomL) > eps && mag(SRight) > 0)
    {
...
void Foam::SWRiemannSolver::computeFlux()
{
    ...
    {        
        ...
        
        {
            ...
            scalarField phLeft = h_.boundaryField()[patchi].patchInternalField();
            scalarField phRight = h_.boundaryField()[patchi];
            if (h_.boundaryField()[patchi].type() == "zeroGradient")
            {
               phRight = phLeft;
            }
                
            scalarField ph0Left = h0_.boundaryField()[patchi].patchInternalField();
            scalarField ph0Right = h0_.boundaryField()[patchi];
            if (h0_.boundaryField()[patchi].type() == "zeroGradient")
            {
               ph0Right = ph0Left;
            }

            vectorField phULeft = hU_.boundaryField()[patchi].patchInternalField();
            vectorField phURight = hU_.boundaryField()[patchi];
            if (hU_.boundaryField()[patchi].type() == "zeroGradient")
            {
               phURight = phULeft;
            }

            const scalarField& pwetdryLeft = wetdry_.boundaryField()[patchi].patchInternalField();
            const scalarField& pwetdryRight = wetdry_.boundaryField()[patchi];

            const scalarField& pNf = Nf_.boundaryField()[patchi];
                
            const scalarField pdX = 2*mag(faceCenter.boundaryField()[patchi] - cellCenter.boundaryField()[patchi].patchInternalField());
